...
const (
	...
	coreGroupName    = ""
	storageGroupName = "storage.k8s.io"
)

var (
	coreSchemeGroupVersion    = schema.GroupVersion{Group: coreGroupName, Version: runtime.APIVersionInternal}
	storageSchemeGroupVersion = schema.GroupVersion{Group: storageGroupName, Version: runtime.APIVersionInternal}
)
...
func withProtectionFinalizer(secret *v1.Secret) *v1.Secret {
	secret.Finalizers = append(secret.Finalizers, SecretProtectionFinalizer)
	return secret
}
...
func TestSecretProtectionController(t *testing.T) {
	secretGVR := coreSchemeGroupVersion.WithResource("secrets").GroupResource().WithVersion("v1")
	podGVR := coreSchemeGroupVersion.WithResource("pods").GroupResource().WithVersion("v1")
	podGVK := coreSchemeGroupVersion.WithKind("Pod").GroupKind().WithVersion("v1")
	pvGVR := coreSchemeGroupVersion.WithResource("persistentvolumes").GroupResource().WithVersion("v1")
	pvGVK := coreSchemeGroupVersion.WithKind("PersistentVolume").GroupKind().WithVersion("v1")
	pvcGVR := coreSchemeGroupVersion.WithResource("persistentvolumeclaims").GroupResource().WithVersion("v1")
	scGVR := storageSchemeGroupVersion.WithResource("storageclasses").GroupResource().WithVersion("v1")
	...
	for _, test := range tests {
		...
		// Create informers
		informers := informers.NewSharedInformerFactory(client, 0 /* NoResyncPeriod */)
